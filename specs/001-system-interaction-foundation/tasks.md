...
- [ ] **T-065 [Test/Fix] 让「会话／面板关闭立即中止回读」在生产调用链上真正成立（MUST，Finding 1）。** **执行结果（2026-08-04）：本任务的输出是「须重开 Plan Gate」，未实施修复，勾选状态保持未完成，待 Reviewer 裁决。** 经生产 bridge 的失败优先测试已交付并取得真实 RED（`Executed 245 tests, with 6 failures`；三个触发——面板关闭、`coordinator.close()`、新会话抢占——全部消耗完 8 次回读与 7 次等待，中止请求无一生效；而 setter 仍为 1、结果未进入 `recoverable`，说明失败性质是「中止压根没及时发生」而非「中止后做错事」）。修复路径穷举四条：端到端异步化、`replace()` 提前返回＋稍后投递、阻塞期间抽运 run loop、`MainActor` 轮询取消标志；**前三条分别触及已批准 Plan `0c9883f` 的数据流、状态机或写入隔离方式，第四条技术上不可行**（`MainActor` 被 `semaphore.wait()` 占住，无代码可置位）。故按本任务约束停止实现。另有一项须 Reviewer 裁决的处理：三项红色测试以**单一开关** `planGateStillPending` + `XCTSkipIf` 入库（skip 消息含任务号、原因与证据路径），以免长期红色阻塞所有 HANDOFF；理由与备选方案见证据第六节。 现状：`GatewaySessionTextTarget.replace/restore` 在 `MainActor` 上以 `DispatchSemaphore.wait()` 同步等待 gateway，`AccessibilityGateway.confirmWrittenText` 又在 actor 内同步 `nanosleep`。回读期间 `MainActor` 无法处理取消、关闭面板或新会话动作，`finishSession()` 触发的 `releaseTarget` 也只能排在被同步休眠占住的 actor 之后。既有 `testTargetInvalidationDuringReadbackStopsImmediately` 只在 fake sleeper 回调里令合成 AX 元素失效，证明的是 A3／A4 外部变化能在下一轮被看到，**没有**证明 tasks.md T-061 条件 (7) 明文要求的 session／panel-close 中止。任务要求，顺序不得颠倒：① **先**增加经过**生产 bridge** 的失败优先测试——在回读进行中结束或替换会话（含关闭面板、`coordinator.close()`、新会话抢占三种触发），断言：**无需等到 deadline** 即返回、剩余回读**不再发生**、setter **不追加**、旧结果**不得进入 `recoverable`**；② 确认该测试为真实 RED 并记录；③ 再修调用链使其转绿。**约束**：不得以「阻塞 actor／MainActor」作为已知限制接受；**若修复会改变已批准的数据流、状态机或写入隔离方式（例如去掉同步 semaphore 桥、把写入路径改为异步、或调整会话状态转移），必须先重开 Plan Gate 再动实现**，不得在本任务内自行变更架构。若判定必须重开 Plan Gate，本任务的正确输出是「须重开 Plan Gate」加上依据，而非绕过。— Depends on: Tasks PASS; Covers: FR-008, FR-010, FR-013, NFR-002, NFR-007; Evidence: `evidence/T-065-readback-session-abort.md`
