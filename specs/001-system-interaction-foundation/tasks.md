...
- [ ] **T-010 [Probe] 验证面板停止条件。** 在单屏、多屏、全屏、边缘位置和非默认缩放下记录结果；若 TextEdit 或 Chrome/ChatGPT 中无法既保持目标又显示完整面板，返回 Gate。— Depends on: T-009; Covers: FR-007, NFR-005, AC-015; Partial Evidence: `evidence/T-010-panel-stop-conditions-partial.md`（多屏、边缘、全屏、非默认缩放及两类目标已通过；等待物理单显示器补测）
...
- PR 审核位置：PR #2
- Implementation Gate：已获用户逐项授权，当前完成 T-001 至 T-009；T-010 已部分执行并等待物理单显示器补测；T-011 及后续任务未获授权
