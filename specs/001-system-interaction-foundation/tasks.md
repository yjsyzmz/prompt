...
stage: tasks
status: revised-pending-review  # Tasks Gate 第四次重开第二版修订（2026-08-04）：Solar 对 `b1cf0f2` 判 CHANGES REQUESTED（四项 MUST、一项 SHOULD），本版已全部落实——P9 唯一执行顺序固定为 T-065→T-066→T-067→T-069→T-070，T-066 的 deadline 边界拆为硬保证与残余风险两层，T-069 样本配额写死为单行 5 + 多行 5（TextEdit 对照各 3 且不计入），T-057 改为「缺 P8 或 P9 任一项」并新增出口 ㉔。共 68 项活动任务。本轮只改 tasks.md；须取得新的 Tasks PASS 才能继续实现，PASS 前不执行 T-062、T-063 与 T-065 至 T-070
...
**P9 对追踪表的影响（2026-08-04，Tasks Gate 第四次重开）**：依 Solar 对 `ad5c9df` 的
REVIEW Finding 3，`5f5f4cf` 的共享回读确认循环已同时接入 replacement 与三条 recovery
路径，**实质改变了 FR-012／AC-013 的恢复行为**。Owner 选择 Finding 3 的「纳入」选项，
因此：**FR-012 与 AC-013 的覆盖新增 T-061 与 T-067**，且**在 T-067 逐条补齐 whole-field
recovery、selected R1、selected R2 fallback 的回读测试之前，FR-012／AC-013 的覆盖不
成立**。FR-008／FR-010／NFR-002／NFR-007 的回读确认边界改由 T-061 加 T-065、T-066、
T-067 共同落实，缺任一项该覆盖不成立。NFR-003 与 AC-005／AC-006 的 Chrome/ChatGPT 闭环
新增 T-069／T-070 及其写回并经新一轮 Tasks Gate `PASS` 的修复任务，**在此之前该闭环
覆盖不成立**（裁决 1：该缺陷属 001 范围，C5 不因适配失败而调整）。
...
- [ ] **T-057 [Evidence] 重建最终验收包并发布 Implementation Gate HANDOFF（REVIEW 要求新增）。** T-050 的产出已整体失效，最终验收改由本任务承担。任务要求：(a) 依 T-052 至 T-056 的实际结果重写 `evidence/T-039-acceptance-package.md`，删除长度元数据的「风险披露」、撤回「结构脚本不存在」的错误更正、更新真实测试计数；(b) 重新逐项核对 13 条 FR、7 条 NFR、17 个 AC，并明确区分哪些证据由真人驱动、哪些由脚本驱动；(c) 重新评估既有 P5／P6 证据的证明力并如实标注受 AXPress 局限影响的项；(d) 五道门禁全部从**分支内** `scripts/` 执行并记录（含 `project-structure-check.sh`）；(e) C3、C4、C5 依实际结果重新判定；(f) 提交推送后发布 Implementation Gate HANDOFF。**完整的最终检查出口（唯一，逐项均须在验收包中留痕）**：① `build.sh` → `BUILD SUCCEEDED`；② `unit-tests.sh` → 全绿，记录真实测试计数；③ `project-structure-check.sh` → passed；④ `sdd-check.sh` → exit 0；⑤ `secret-scan.sh` → 无命中；⑥ `git diff --check` → 无输出；⑦ 以上六项全部从**分支内** `scripts/` 执行，不得使用主 worktree 的脚本；⑧ 诊断输出经 `log stream` 实测确认不含任何长度或内容度量；⑨ 13 条 FR、7 条 NFR、17 个 AC 逐项核对且每项标明证据来源为真人驱动或脚本驱动；⑩ C3、C4、C5 依实际结果重新判定并写明依据；⑪ 已知限制清单重建，删除长度元数据「风险披露」与「结构脚本不存在」两处错误记述；⑫ **`unit-tests.sh` 须连续执行两次且两次全绿、计数一致**，以排除偶发；⑬ **所有门禁命令的执行位置须记录为 `.worktrees/fable` 下的分支内 `scripts/`（即 `bash scripts/<name>.sh`），禁止使用 `../../scripts/` 指向主 worktree**；⑭ **同步更新 PR #2 的描述**：head SHA 改为本次 HANDOFF 的准确 SHA、任务总数改为 **62**（T-001 至 T-058 共 58 项，加 T-061 至 T-064 共 4 项；原 T-059／T-060 已移出任务列表）、并移除仍指向 `9bb221e` 与 46 个任务的过时表述。 **2026-08-03 追加（P8）**：⑮ T-058 与 T-061 的证据已归档；⑯ **T-063 与 T-064 的证据已归档**，且 T-064 写回并经新一轮 Tasks Gate `PASS` 的 **ChatGPT 修复任务的证据亦已归档**；⑰ T-062 的完整 T-055 重跑结果已归档且逐项标注真人驱动，含二次替换回归项与按尝试逐条列表；⑱ 已知限制清单补入「Google 搜索框选区兼容已移出 001，见 `evidence/out-of-scope-observations.md` 观察 5」。任一项未留痕即不得发布 HANDOFF。 **2026-08-04 追加（P9）**：⑲ T-065、T-066、T-067、T-068 的证据已归档，且 T-061 的四项 MUST 均已闭合；⑳ T-069 与 T-070 的证据已归档，且 T-070 写回并经**新一轮 Tasks Gate `PASS`** 的首次写入修复任务的证据亦已归档；㉑ 验收包须逐项区分「ChatGPT 首次写入」与「ChatGPT 二次替换」两条不同路径的证据，不得互相顶替；㉒ 任务总数改为 **68**（T-001 至 T-058 共 58 项，加 T-061 至 T-070 共 10 项；原 T-059／T-060 已移出任务列表），⑭ 中的「62」以此为准更正；㉓ 已知限制清单补入 T-066 允许披露的残余风险「单次已经开始的 AX 调用不可中断，可能越过 deadline，因此整个 readback loop 的实际 elapsed 无硬上界保证」，以及 T-065 若重开 Plan Gate 的处置结果；**㉔ 依 Solar REVIEW（`b1cf0f2`）SHOULD 5，T-058 遗留的生产 assembly test seam 必须在此二选一并留痕，不得静默消失**：要么补上真正经过 `AppLifecycleController.convenience init()` 的装配断言（同时覆盖 replacement 与 stale-target 两个诊断 recorder），要么记录延期理由并建立对应的后续 Issue 并在验收包中给出 Issue 编号。— Depends on: T-052, T-053, T-054, T-056, **以及 P8 全部完成**——T-058、T-063、T-064、T-061、T-062，以及 T-064 写回并经新一轮 Tasks Gate `PASS` 的 ChatGPT 修复任务，**再加 P9 全部完成**——T-065、T-066、T-067、T-068、T-069、T-070，以及 T-070 写回并经新一轮 Tasks Gate `PASS` 的首次写入修复任务。**T-055 不再单独作为 T-057 的前置**：其证据已由 T-062 在最终代码上重取取代。缺 P8 **或 P9** 任一项即不得发起最终 Implementation Gate 审核，**并且 T-064 与 T-070 各自写回、各自经新一轮 Tasks Gate `PASS` 并已实施完成的修复任务同样是本任务的前置**——两条修复链缺任一条亦不得发起最终审核。; Covers: FR-001 至 FR-013, NFR-001 至 NFR-007, AC-001 至 AC-017; Evidence: `evidence/T-039-acceptance-package.md`
...
**第四版修订（2026-08-03）。** Solar 对 `03a9f0b` 的快速复核判 `CHANGES REQUESTED`，
三项文档缺口，本版已全部落实：① T-061 补入第三条必备失败优先测试「回读中途
session／target 失效须立即停止、不得等到 deadline、不得追加写入，并断言剩余回读
次数未被消耗」，Covers 补入漏标的 **FR-010**；② T-063 的样本数由「至少 10 次」改为
**恰好 10 次**（固定样本数；中途因环境中断须重置并重新计满，废弃批次须记录）；
③ T-057 的任务总数由 60 更正为 **62**（实测：`grep` 去重计数为 62 = T-001 至 T-058
共 58 项 + T-061 至 T-064 共 4 项；原 T-059／T-060 已移出），并把 T-063、T-064
及 T-064 写回后经新一轮 Tasks PASS 的 ChatGPT 修复任务的证据一并列入归档出口。
...
- [ ] **T-063 [Probe/Evidence] 在固定环境与固定次数下复现 ChatGPT 二次替换拒绝（MUST，当前暂停）。** **状态（2026-08-04，依 Solar 对 `ad5c9df` 的 NEEDS DECISION 裁决 3）**：本任务**暂停**。现状下 ChatGPT 连第一次替换都不成功（`5f5f4cf` 上三条 `readback failure=writeFailed`，写入始终未落地），二次替换场景**不可达**；**不得把不可达场景硬跑成 10 次失败，也不得用第一次失败顶替二次样本**。**新增可达性前置（唯一，全部满足方可开工）**：① T-069 完成采集、T-070 完成判定；② T-070 写回的首次写入修复任务经**新一轮 Tasks Gate `PASS`** 并实施完成；③ 在真机上由真人完成**至少一次**「确认替换 → 恢复原文」全程成功（须留下 `replacement-stage=completed` 记录与恢复成功的逐项记录）。上述三项齐备后才执行下列采集。完成条件：① 记录 Chrome 与 ChatGPT 的**具体版本号**、macOS 版本、应用 SHA 与 cdhash；② 使用**固定的合成夹具**（`SYNTHETIC-001` 标记，文本内容写入证据文件，保证可复现）；③ 由真人以鼠标／触控板复现二次替换场景**恰好 10 次**（固定样本数，不多不少；若中途因环境问题中断，须重置环境并重新计满 10 次，并在证据中记录被废弃的批次）——**环境固定的含义已按实测补明**：桌面上除目标应用外不得有其他前台窗口（含系统设置与 Finder），采集过程中不得切换应用，否则该批次作废；每次记录：尝试编号、是否为二次替换、面板文案原文、监听器判定依据的原因分类、以及该次是否产生替换阶段记录；④ Google 搜索框**仅作范围外对照**，各跑 3 次并单独成表，不参与 001 的结论；⑤ 逐次记录**不得含任何内容或长度**，只记原因分类与结果；⑥ 若首轮 10 次中仍有 0 次进入二次替换，证据的正确输出是**「二次场景不可达」**并说明卡在哪个阶段，**不得伪造样本**。**本任务只采集，不分析根因、不改任何实现。** — Depends on: T-058, **T-069, T-070, 以及 T-070 写回并经新一轮 Tasks Gate `PASS` 的首次写入修复任务实施完成，且真机上首次「替换 → 恢复」已成功**; Covers: FR-013, NFR-006, NFR-007; Evidence: 新增 `evidence/T-063-second-replacement-probe.md`
...
- [ ] **T-061 [Fix] 把回读确认改为有界、单次 setter、超时 fail-closed（MUST）。** **状态（2026-08-04）**：首版实现已落地于 `5f5f4cf`（证据 `evidence/T-061-readback-confirmation-bound.md`），但 Solar 对 `ad5c9df` 判 `CHANGES REQUESTED`，四项 MUST 未闭合，**本任务因此不算完成**，勾选状态清空。已确认成立的部分（Solar Findings 5、6 ACCEPTED）：执行顺序调整合理；`ReplacementStage.readbackAborted` 属内容无关诊断细分，不需单独重开 Plan Gate。**本任务的完成条件现在追加为：T-065、T-066、T-067 全部完成，且 T-068 的证据更正已归档。** T-055 抓到 3 条 `replacement-stage=readback failure=writeFailed`：setter 报告成功但回读未确认，现有五次重试预算耗尽。这与 T-058 观测的拒绝是不同问题（那些从未到达 setter）。已知事实：Chromium 异步应用写入，立即回读返回写入前的值（T-037 已记录）。**依 Solar Tasks Gate REVIEW 更正引用**：本任务与 AC-008、NFR-001 无关——NFR-001 的 300ms 只约束确认前的预览呈现，**不包含确认之后的回读**，因此不存在需要协商的时间上限。任务要求：先写失败测试固定下列**全部**条件，再改实现。**回读预算**：(1) 使用**单调时钟**（`ContinuousClock` 或 `DispatchTime`）计算 deadline，**不得使用墙钟**，以免系统时间调整影响预算；(2) 回读之间采用**有界退避**，次数与总时长均有明确上界并写入证据文件。**比较**：(3) 每次回读须逐 **UTF-16 code unit** 与 expected result 比较相等，不得用规范化比较、前缀比较或长度比较代替。**写入约束**：(4) **setter 只调用一次**，重试只重复回读；(5) **不得换用其他 setter**（例如从 `kAXSelectedText` 改为 `kAXValue`）作为"重试"手段；(6) **不得把 setter 返回成功视为最终成功**——唯一的成功判据是回读一致。**中止与后备**：(7) 回读期间若 session 或 target 失效（含面板关闭、目标应用退出、窗口／元素身份变化），必须**立即停止**回读，不得继续等待 deadline；(8) 达到 deadline 仍不一致时 **fail-closed**，报告写入未确认，并给出安全后备动作（保留结果可复制、不声称已写入、不自动重试写入）。**隐私**：(9) 回读诊断只记阶段与结果，**不得记录内容或长度**（沿用 T-052 边界）。**必须包含的三条失败优先测试**：(a) 「延迟后最终一致」——setter 后前若干次回读不一致、deadline 内某次一致，须报成功且 setter 仅调用一次；(b) 「直到 deadline 仍不一致」——须 fail-closed 报未确认，且不得追加写入；(c) **「回读中途 session／target 失效」**——在回读循环进行中令 session 或 target 失效（面板关闭、目标应用退出、窗口或元素身份变化），须**立即停止回读**而非等到 deadline，报告未确认，且不得追加任何写入；该测试须断言剩余回读次数确实未被消耗。**不得以放弃回读确认的方式让测试通过。** — Depends on: Tasks PASS; **完成还须 T-065、T-066、T-067、T-068**; Covers: FR-008, FR-010, FR-013, NFR-006, NFR-007, **FR-012, AC-013**（依 Finding 3 的选项二纳入，见 T-067）; Evidence: `evidence/T-061-readback-confirmation-bound.md`
- [ ] **T-062 [Verify] 在最终代码上重跑完整 T-055（MUST）。** **依 Solar Tasks Gate REVIEW 更正范围**：不是只补九个未执行项，而是在 T-058、T-061 及后续新一轮 Tasks Gate 定义的修复全部落地后的**最终代码**上，由真人以鼠标／触控板重跑 T-055 的**全部**内容——(a) TextEdit 连续五次、(b) Chrome/ChatGPT 连续五次多行、(c) Undo、(d) 范围保真、(e) 恢复后外部变化、(f1) 至 (f6)。此前在 `a645769` 上取得的 (a)(b) 结果因代码已变更而不再具备证明力，须重取。逐项记录面板文案原文与对应阶段记录；时序约束不变，f3／f5／f6 必须发生在替换成功之后、恢复之前；禁止以 AXPress 或 AppleScript 代点。**追加：必须包含 ChatGPT 二次替换的回归项**——在同一输入框连续完成两轮「确认替换 → 恢复原文」，第二轮必须成功；该项与 (b) 的五次多行分别记录，不得互相顶替。**证据须按尝试逐条列出**：尝试编号、目标应用、是否二次替换、是否重试、最终结果、以及该次对应的阶段记录条数，并使闭环次数、`completed` 条数与失败条数三者的算术关系在文中显式成立。— Depends on: T-058, T-063, T-064, T-061, **T-065、T-066、T-067、T-068、T-069、T-070**, 以及 T-064 与 T-070 分别写回 tasks.md 并各自经**新一轮 Tasks Gate `PASS`** 的修复任务（缺任一项即不得开工）; Covers: FR-007, FR-008, FR-012, NFR-007, AC-005, AC-006, AC-012, AC-013, AC-017; Evidence: `evidence/T-055-human-driven-revalidation.md`

### P9 — T-061 修订与 ChatGPT 首次写入诊断（2026-08-04，Tasks Gate 第四次重开，当前待审）

**本段的来源与边界。** Solar 对 `ad5c9df` 的 T-061 单任务审查判 `CHANGES REQUESTED`，
四项 MUST 加两项 ACCEPTED，并对此前提出的 NEEDS DECISION 作出三条裁决。本段**只定义
任务与证据出口，不预设根因、不预设修复方案**；Tasks PASS 前不动生产代码，不执行
T-063、T-062，也不开发任何 ChatGPT 修复。

**已被裁决固定的三件事**，本段据此展开：① 「ChatGPT 写入不落地」**属于 Feature 001
范围**（NFR-003 要求 Chrome/ChatGPT 完成读取、预览、确认、替换与恢复，AC-005／AC-006／
AC-012／AC-013 与 C5 均依赖该闭环），C5 保持未达成，**不得为适配当前失败而调整**；
② 该缺陷**不得未经 Gate 并入 T-064**，须独立定义 Probe 与 Decision 任务；③ T-063 暂停，
须建立可达性依赖。

**P9 的唯一执行顺序（依 Solar 对 `b1cf0f2` 的 REVIEW MUST 1 固定，不得并行、不得调换）：**

```text
T-065
  └─（若 T-065 判定必须重开 Plan Gate）→ 完成该 Plan Gate → 完成随后的 Tasks Gate
        └─ T-066
              └─ T-067
                    └─ T-069 → T-070
                          └─（T-070 写回并经新一轮 Tasks Gate PASS 的首次写入修复实施完成）
                                └─ T-063 → T-064 → T-062 → T-057
```

理由：T-065 可能改变同步 bridge、回读等待与会话取消架构，先做 T-066／T-067 会被返工；
而 T-067 的 session／target 失效测试本身依赖 T-065 建立的**真实中止能力**，缺它则该测试
无从断言。T-068 无依赖，已在本轮完成。

- [x] **T-065 [Test/Fix] 让「会话／面板关闭立即中止回读」在生产调用链上真正成立（MUST，Finding 1）。** **执行结果（2026-08-04，第二版）：已修复并转绿，246 项测试、0 skipped、0 failures。** Owner 上一版（`005ca4c`）判定「须重开 Plan Gate」**是错误的，已撤回**：Solar 对 `005ca4c` 的 REVIEW Finding 1 给出行号，Owner 核对 `plan.md` 后确认——第 130-135 行规定 `TextTargetAccessing.replace／restore／release` 全是 `async`，第 55 行要求异步结果按 session ID 去重，第 158 行已含 `applying`，第 387 行要求会话结束时取消异步任务。**真正偏离已批准 Plan 的是被移除的同步协议与 `DispatchSemaphore` 桥**，因此本修复是 Plan 符合性修复，不需重开 Plan Gate。错误原因：Owner 从实现代码与任务约束文本推理，未先读 `plan.md` 接口章节。修复四处：① 协议恢复 `async` 并留在 `@MainActor`，删除 `performBlocking`／`ResultBox`；② 协调器保存 `applyWork`／`recoverWork` 并在 `endSession`／`replaceCurrentSession` 取消；③ 结果回到 `MainActor` 后按启动时捕获的 session ID 与当前状态 fail-closed，取消后的结果不得进入 `recoverable`；④ 回读循环每轮前检查 `Task.isCancelled`。**负向对照**：把 ④ 临时改为永不成立，恰好三项中止测试失败（`8 !< 8`、`7 !< 7`），其余 243 项不受影响，证明 GREEN 归因于取消机制而非调度运气。测试按 Finding 3 改为确定性握手（信号量 + `withCheckedContinuation`，不占用 `MainActor`），并新增第四项断言「被取消的写入即便随后成功也不得进入 `recoverable`」。`planGateStillPending` 开关与全部 skip 已按 Solar 裁决整体删除。 现状（修复前）：`GatewaySessionTextTarget.replace/restore` 在 `MainActor` 上以 `DispatchSemaphore.wait()` 同步等待 gateway，`AccessibilityGateway.confirmWrittenText` 又在 actor 内同步 `nanosleep`。回读期间 `MainActor` 无法处理取消、关闭面板或新会话动作。既有 `testTargetInvalidationDuringReadbackStopsImmediately` 只在 fake sleeper 回调里令合成 AX 元素失效，证明的是 A3／A4 外部变化能在下一轮被看到，**没有**证明 tasks.md T-061 条件 (7) 明文要求的 session／panel-close 中止。任务要求，顺序不得颠倒：① **先**增加经过**生产 bridge** 的失败优先测试——在回读进行中结束或替换会话（含关闭面板、`coordinator.close()`、新会话抢占三种触发），断言：**无需等到 deadline** 即返回、剩余回读**不再发生**、setter **不追加**、旧结果**不得进入 `recoverable`**；② 确认该测试为真实 RED 并记录；③ 再修调用链使其转绿。**约束**：不得以「阻塞 actor／MainActor」作为已知限制接受。— Depends on: Tasks PASS; Covers: FR-008, FR-010, FR-013, NFR-002, NFR-007; Evidence: `evidence/T-065-readback-session-abort.md`
- [x] **T-066 [Test/Fix] 把 deadline 收紧为「不得在 `now >= deadline` 时开始新回读」（MUST，Finding 2）。** **执行结果（2026-08-19）：已修复并转绿。第二版闭合 Solar 对 `e22a4fa` 的 Finding 1：A1／A3／A4 各自采样 deadline，三项跨线测试先 RED 后 GREEN。** 现状：1200ms deadline 只约束休眠累计，不约束「何时允许开始下一次回读」——代码在回读失败后才检查时钟，可能刚好休眠到 deadline，下一轮仍会先执行 A1／A3／A4 与 AX readback；真实 AX 查询耗时完全落在预算之外。既有 `testTheDeadlineStopsTheLoopIndependentlyOfTheAttemptCeiling` 还把「到达 300ms deadline 后执行第 4 次回读」**写成了期望结果**，该断言固定的是错误行为，须一并更正。**依 Solar 对 `b1cf0f2` 的 REVIEW MUST 2，边界拆成两个层次，测试须分别对应，不得一边断言总 elapsed 绝不超限、一边允许它超限：**
  - **层次一——硬保证（必须以失败优先测试断言）**：① **最大尝试次数**有上界；② **累计等待预算**（各次休眠之和）有上界；③ **`now >= deadline` 之后不得开始任何新的 A1／A3／A4 检查或 AX readback**，即不得再消费回读次数。这三项是无条件成立的硬约束。
  - **层次二——残余风险（只记录，不作为上界断言）**：**单次已经开始的 AX 调用不可中断，可能越过 deadline**，因此整个 readback loop 的实际 elapsed **不作硬上界保证**；须在证据中记录实测 elapsed 并如实披露该风险。**除非另有 AX 调用级 timeout**，否则不得声称总 elapsed 有硬上界。
  - **明确取消**上一版写入的「整个 loop 的 elapsed 以单调时钟测量且有界」这一条：它与层次二矛盾，按 REVIEW 判定作废。
  — Depends on: **T-065 的最终通过结果**（若 T-065 触发 Plan Gate 重开，则须待该 Plan Gate 与随后的 Tasks Gate 均 `PASS` 且 T-065 完成之后才可开工——T-065 可能改变同步 bridge、回读等待与会话取消架构，先做本任务会被返工）; Covers: FR-008, FR-010, NFR-007; Evidence: 新增 `evidence/T-066-readback-deadline-strict.md`
- [x] **T-067 [Test] 补齐三条 recovery 路径的回读行为覆盖，并把 FR-012／AC-013 正式纳入 T-061（MUST，Finding 3）。** **执行结果（2026-08-19）：首版 12 项路径覆盖被 Solar 对 `0abf88c` 判 CHANGES REQUESTED。本轮闭合两项 MUST 与一项 SHOULD：R1 回读改为 selected-attribute-only（失败优先：selected 错、whole substring 对 → fail-closed 且 `fullValueReadCount == 0`）；W4／R1／R2 各增一条生产 `recoverWork` 取消测试； (b) 断言默认 deadline 恰好耗尽（7 次回读／1200ms）。全套 272 tests / 0 failures。未开工 T-069。** 事实：`5f5f4cf` 把新的 8 次／1200ms 确认循环同时接入了 replacement 与三条 recovery 写后回读路径（whole-field W4、selected R1、selected R2 fallback），**实质改变了 FR-012／AC-013 的恢复行为**，而 T-061 原 Covers 不含 FR-012／AC-013，新测试也只覆盖 replacement。**Owner 在 Finding 3 的二选一中选择「纳入」而非「限回 replacement」，理由如下**：限回 replacement 需要把三条 recovery 路径退回旧的「5 次固定 100ms + Swift `String ==`」循环，那会在恢复路径上**保留已被证实的规范等价比较缺陷**（规范等价不是相等，见 T-061 证据条件 (3)），是安全性更差的选项。该选择只是覆盖范围的决定，**不预设任何根因或修复方案**。任务要求：为**三条 recovery 路径逐条**补齐失败优先测试——(a) 延迟后最终一致（setter 恰好 1 次）；(b) 直到 deadline 仍不一致须 fail-closed 且不追加写入；(c) 回读中途 session／target 失效须立即中止且剩余次数未被消耗；(d) 每条路径只回读其实际写入的属性，**不得换用另一个 setter 或另一条路径的回读策略**。三条路径各自独立记录，不得以一条顶替另一条。**共享 helper 不得再悄悄扩大已批准任务的行为面**：本任务须在证据中列出该 helper 的全部调用点与各自覆盖的需求。— Depends on: **T-065（其 session／target 失效测试依赖 T-065 建立的真实中止能力，缺它则本任务的 (c) 项无从断言）与 T-066（deadline 语义确定后才能固定各路径断言）**，二者均须先最终通过; Covers: FR-008, FR-010, **FR-012**, NFR-002, NFR-007, **AC-013**; Evidence: 新增 `evidence/T-067-recovery-readback-coverage.md`
- [x] **T-068 [Evidence] 删除 T-061 证据中超出证据的定性（MUST，Finding 4）。** T-061 证据原第 314–316 行写有「写入路径本身完好」「定性为**目标特定**，而不是本项目写入实现的缺陷」。该定性超出现有证据：TextEdit 两次成功只能证明共享写入路径在 TextEdit 可用，ChatGPT 特有的 capture mode、属性选择、range 语义或适配逻辑**仍可能是本项目的实现缺陷**。完成条件：① 删除该定性；② 保留的结论限定为——「失败可在 ChatGPT 复现、TextEdit 对照未复现；扩大 readback budget 不能使这三次写入落地；**根因未知**」；③ 明确写明 T-064 与 T-070 **不得继承**被删除的推断。**本轮已执行**（属 SDD 制品修改，不涉生产代码）。— Depends on: 无; Covers: 无新增需求（证据更正）; Evidence: `evidence/T-061-readback-confirmation-bound.md` 第 7 条
- [ ] **T-069 [Probe/Evidence] 在固定环境下采集 ChatGPT「首次写入不落地」（MUST，裁决 2）。** 与 T-063 是**两条不同路径**：T-063 采的是监听器路径拒绝（根本不到 setter），本任务采的是 **setter 之后**的写入未落地。已知的最窄事实（`5f5f4cf`，macOS 26.5.2／25F84，Chrome 150.0.7871.187，cdhash `26aef136`）：三次 `replacement-stage=readback failure=writeFailed`，**无一条 `readbackAborted`**，真人不点任何按钮等待数秒后 `【系统交互验证】` 前缀始终未出现；TextEdit 对照两次 `completed`。完成条件：① 记录 Chrome 与 ChatGPT 的具体版本号（若网页版无用户可见版本号，须如实写明「无可见版本号」并记录访问日期）、macOS 版本、应用 SHA 与 cdhash；② 固定合成夹具（`SYNTHETIC-001` 标记，文本写入证据文件），**单行与多行两种夹具的样本数依 Solar REVIEW MUST 3 预先写死，开工后不得调整**：**ChatGPT 单行恰好 5 次 + ChatGPT 多行恰好 5 次，合计恰好 10 次**；③ 由真人以鼠标／触控板在 ChatGPT 输入框按上述配额执行首次替换，逐次记录：尝试编号、夹具类别（单行／多行）、捕获模式（选区／整段）、面板文案原文、阶段记录序列（须含 `setter`／`readback`／`readbackAborted`／`completed` 的实际出现情况）、以及**写入是否在事后落地**（等待固定秒数后由真人目视确认，秒数写入证据）；④ **TextEdit 对照：单行 3 次 + 多行 3 次，合计 6 次，单独成表，且明确不计入 ChatGPT 的 10 次**；⑤ 环境固定同 T-063：除目标应用外无其他前台窗口，采集中不切换应用，违反即整批作废并记录；⑥ 逐次记录**不得含任何内容或长度**，只记类别、模式、阶段与结果。**两类夹具各自独立成表，任一类不足配额即整批作废重取，不得以一类顶替另一类。****本任务只采集：不分析根因、不改任何实现、不预设是替代 setter／属性策略／捕获模式还是架构问题。** — Depends on: Tasks PASS, T-065, T-066, T-067（须在 T-061 修订完成后的代码上采集，否则样本不代表最终行为）; Covers: FR-008, FR-010, FR-012, NFR-003, NFR-006, NFR-007; Evidence: 新增 `evidence/T-069-chatgpt-first-write-probe.md`
- [ ] **T-070 [Decision] 依 T-069 证据判定首次写入不落地的根因并把修复任务写回 tasks.md（MUST，裁决 2）。** 完成条件：① **只依据 T-069 的记录**判定，不得引入未采集的推断，**不得继承 T-068 删除的那条定性**；② 判定该根因是否触及已批准的 Plan `0c9883f`——**若证据指向替代 setter、属性策略、捕获模式或架构变化，必须先重开 Plan Gate**，不得直接改实现；③ 列出所需的失败优先测试与**最小**修复范围，明确哪些情形应被容忍、哪些必须继续拒绝，**不得为让 ChatGPT 通过而放宽写入安全约束**（不得取消回读确认、不得增加第二种 setter、不得以模拟粘贴替代 AX 写入）；④ 把具体 Fix 任务（含依赖、覆盖、完成条件、Evidence 出口）写回 `tasks.md`；⑤ 提交新 SHA 并发起**新一轮 Tasks Gate HANDOFF**。**新 Tasks Gate `PASS` 之前不得实施任何 ChatGPT 修复。** 若 T-069 的证据不足以支撑判定，本任务的正确输出是**「证据不足」**并补充采集，不得以推断填补。— Depends on: T-069; Covers: 无新增需求（决策与任务定义）; Evidence: 新增 `evidence/T-070-chatgpt-first-write-root-cause.md`
...
- **C3 — 系统边界可控：** T-028 通过；权限、剪贴板、AX、observer、几何和预览各自有测试，并且权威写入验证未被监控事件取代。~~2026-07-28 起回退为未达成~~ **已于 2026-07-30 重新达成**：T-021／T-022 按批准算法转绿，T-040 自动化复核确认权限、剪贴板、AX、observer、几何与预览各套件全部通过。 **2026-07-31 再次回退为未达成**：Tasks Gate 第二次重开使下游 Implementation Gate 一并重开；且第三次 Implementation REVIEW 的 Finding 3 判定 A2 豁免超出批准范围，系统边界的判定依据本身需按 T-053 重做。 **2026-08-03 追加（P8）**：重新达成还须 T-058 的监听器路径诊断与 T-061 的回读确认边界完成，且 T-064 判定的根因若触及已批准 Plan 则先重开 Plan Gate；缺任一项 C3 不得判定达成。 **2026-08-04 追加（P9）**：T-061 的首版实现被判 `CHANGES REQUESTED`，其中 Finding 1 指出「会话／面板关闭立即中止」在生产调用链上不成立（`MainActor` 被同步 semaphore 与 actor 内同步休眠阻塞），**这是系统边界本身的缺口，不是可接受限制**。C3 重新达成追加条件：⑨ T-065 完成且其修复未绕过 Plan——若修复触及已批准的数据流、状态机或写入隔离方式，须先重开 Plan Gate；⑩ T-066 的严格 deadline 完成；⑪ T-067 的三条 recovery 路径覆盖完成。缺任一项 C3 不得判定达成。
- **C4 — 合成闭环通过：** T-031 通过；全部生产装配由 T-030 的端到端失败测试驱动。~~2026-07-28 起回退为未达成~~ **已于 2026-07-30 重新达成**：T-040 的两次运行中 T-027 与 T-030／T-031 套件全部通过。 **2026-07-31 再次回退为未达成**：Tasks Gate 第二次重开使下游 Implementation Gate 一并重开；且该闭环全部由 AXPress 驱动，不覆盖预览面板成为 key window 的真人路径，需按 T-055 以真人驱动重新建立。 **2026-08-03 追加（P8）**：真人驱动的闭环证据改由 **T-062 在最终代码上重取**，`a645769` 上的 T-055 结果不再计入；且必须包含 ChatGPT 二次替换回归项通过。缺 T-062 即 C4 不得判定达成。 **2026-08-04 追加（P9）**：T-062 的前置已扩充为还须 T-065 至 T-070 全部完成，以及 T-064 与 T-070 各自写回并经新一轮 Tasks Gate `PASS` 的修复任务落地；在 ChatGPT 首次写入能够落地之前，C4 的真人闭环不可能取得，C4 不得判定达成。
- **C5 — 真实验收完整：** T-039 通过；TextEdit 与 Chrome/ChatGPT 完整闭环、VS Code 后备闭环、性能/显示/输入/隐私证据均可复现。 ~~已达成（2026-07-28）~~ ~~已回退为未达成~~ **已于 2026-07-30 重新达成**：T-040 至 T-046 全部完成，含 P6 的真实环境复核与定向审计，详见 `evidence/T-039-acceptance-package.md` 第十节。原记录：121 tests 全绿，build/structure/sdd/secret/diff-check 五道门禁通过，7 项已知限制已如实披露，详见 `evidence/T-039-acceptance-package.md`。 **2026-07-31 再次回退为未达成**：Solar 对 `5070607` 的第三次 Implementation Gate REVIEW 判定 `CHANGES REQUESTED`，6 项 MUST 未闭合；且 Tasks Gate 因 P7 重开，下游 Implementation Gate 随之重开。**重新达成的条件（唯一，全部满足方可）**：① T-052 至 T-056 全部完成；② T-055 的真人复核全部通过——TextEdit 与 Chrome/ChatGPT 各连续五次「确认替换 → 恢复原文」成功，Undo 行为、范围保真、恢复后外部变化、三类关键失败场景均有真人记录；③ 五道门禁全部从分支内 `scripts/` 执行并通过（含 `project-structure-check.sh`）；④ T-057 重建的验收包逐项区分真人驱动与脚本驱动证据，并标注受 AXPress 局限影响的项；⑤ Solar 对 T-057 发布的新 SHA 给出 Implementation Gate `PASS`。 **2026-08-03 追加（P8）**：⑥ T-058 与 T-061 完成；⑦ T-058 的诊断数据取得后，ChatGPT 二次替换的修复任务经**新一轮 Tasks Gate** 定义并完成——不得在本次 PASS 后临时定义即开发；⑧ T-062 在最终代码上重跑**完整** T-055 并全部通过（此前 `a645769` 上的 (a)(b) 结果已失效，不再计入）。Google 搜索框选区兼容依 `BLOCKER` 移出 001，不作为 C5 条件。 **2026-08-04 追加（P9）**：依裁决 1，「ChatGPT 首次写入不落地」属 001 范围，**C5 保持未达成，不得为适配当前失败而调整**；只有用户明确决定删减产品承诺时才重开 Spec Gate，Reviewer 不建议删减。重新达成追加条件：⑨ T-065、T-066、T-067 完成，T-061 因四项 MUST 闭合而真正完成；⑩ T-069 采集与 T-070 判定完成，且 T-070 写回并经**新一轮 Tasks Gate `PASS`** 的首次写入修复任务实施完成；⑪ 真机上 ChatGPT 的「确认替换 → 恢复原文」至少完成一次全程成功（这也是 T-063 的可达性前置）；⑫ T-068 的证据更正已归档，且 T-064／T-070 均未继承被删除的定性。
...
- **第三次重开第四版修订（2026-08-03，当前待审）**：Solar 对 `03a9f0b` 的快速复核
  判 `CHANGES REQUESTED`，三项文档缺口。已落实：T-061 补入第三条必备失败优先测试
  （回读中途 session／target 失效须立即停止）并补标漏掉的 FR-010；T-063 的样本数
  由「至少 10 次」改为「恰好 10 次」；T-057 的任务总数由 60 更正为 62（实测计数
  确认），并把 T-063、T-064 及新一轮 Tasks PASS 后的 ChatGPT 修复任务证据列入
  归档出口。本轮仅修改 `tasks.md`。
- **第四次重开（2026-08-04，当前待审）**：Solar 对 `ad5c9df` 的 T-061 单任务审查判
  `CHANGES REQUESTED`，四项 MUST 加两项 ACCEPTED，并对 Owner 提出的 NEEDS DECISION
  给出三条裁决。本轮**只修改 SDD 制品**，未动一行生产代码，也未执行 T-063、T-062，
  未开发任何 ChatGPT 修复。已落实：
  - **T-061 勾选状态清空**，四项 MUST 未闭合前不算完成；完成条件追加 T-065 至 T-068。
  - **新增 P9（T-065 至 T-070）**：T-065 对应 Finding 1（经生产 bridge 的 session／
    面板关闭中止，若修复触及已批准数据流／状态机／写入隔离则先重开 Plan Gate）；
    T-066 对应 Finding 2（严格 deadline：除首次外不得在 `now >= deadline` 时开始回读，
    并更正把错误行为写成期望的既有断言）；T-067 对应 Finding 3（三条 recovery 路径
    逐条补齐回读覆盖）；T-068 对应 Finding 4（删除超出证据的定性，本轮已执行）；
    T-069／T-070 对应裁决 2（ChatGPT 首次写入不落地的独立 Probe 与 Decision）。
  - **Finding 3 的二选一：Owner 选择「纳入 FR-012／AC-013」**，理由是限回 replacement
    需把三条 recovery 路径退回旧的「5 次固定 100ms + Swift `String ==`」循环，会在恢复
    路径上保留已被证实的规范等价比较缺陷，是安全性更差的选项。该选择只决定覆盖范围，
    不预设根因或修复方案。
  - **T-063 暂停**并建立可达性依赖（依裁决 3）：须先经 T-069／T-070 与新一轮 Tasks
    PASS 的首次写入修复落地、且真机首次「替换 → 恢复」成功，才执行恰好 10 次二次
    替换采集；首轮若 0 次可达，正确输出是「二次场景不可达」，不得伪造样本。
  - 追踪表补入 P9 影响；C3 追加 ⑨⑩⑪、C4 追加 P9 段、C5 追加 ⑨⑩⑪⑫；T-057 追加出口
    ⑲ 至 ㉓ 并把任务总数由 62 更正为 **68**（实测去重计数：T-001 至 T-058 共 58 项 +
    T-061 至 T-070 共 10 项，原 T-059／T-060 已移出）。
  - 依裁决 1，「ChatGPT 首次写入不落地」属 001 范围，**C5 保持未达成**，不为适配当前
    失败而调整；是否删减产品承诺只能由用户决定并重开 Spec Gate，Reviewer 不建议删减。
- **第四次重开第二版修订（2026-08-04，当前待审）**：Solar 对 `b1cf0f2` 判
  `CHANGES REQUESTED`，四项 MUST 加一项 SHOULD。本轮**只改 `tasks.md`**，未动源码、
  测试与证据文件。已落实：
  - **MUST 1（依赖顺序仍允许返工）**：新增「P9 的唯一执行顺序」图并固定为
    T-065 →（若触发则 Plan Gate 与随后的 Tasks Gate）→ T-066 → T-067 → T-069 →
    T-070 →（新一轮 Tasks PASS 的首次写入修复）→ T-063 → T-064 → T-062 → T-057；
    T-066 的 Depends on 由「Tasks PASS」改为「T-065 的最终通过结果」，T-067 改为
    依赖 T-065 与 T-066 两者。
  - **MUST 2（T-066 两项完成条件自相矛盾）**：把边界拆为两个层次。硬保证只有三项
    ——最大尝试次数、累计等待预算、`now >= deadline` 后不得开始任何新的 A1／A3／A4
    或 AX readback；「整个 loop 的 elapsed 有硬上界」这一条**明确作废**，改为残余风险
    层次：单次已开始的 AX 调用不可中断、可能越过 deadline，只记录实测 elapsed，除非
    另有 AX 调用级 timeout 才可声称硬上界。测试须分层对应。
  - **MUST 3（T-069 样本配额有歧义）**：预先写死为 ChatGPT 单行 5 次 + 多行 5 次
    合计 10 次；TextEdit 对照单行 3 次 + 多行 3 次合计 6 次，单独成表且明确不计入
    ChatGPT 的 10 次；两类各自独立成表，任一类不足配额即整批作废重取。
  - **MUST 4（T-057 末尾仍只写「缺 P8」）**：改为「缺 P8 **或 P9** 任一项」，并补入
    T-064 与 T-070 两条修复链各自写回、各自经新一轮 Tasks Gate `PASS` 并实施完成
    同为 T-057 前置。
  - **SHOULD 5（T-058 遗留的生产 assembly test seam）**：新增 T-057 出口 ㉔，二选一
    并须留痕——补上真正经过 `convenience init()` 的装配断言（同时覆盖两个诊断
    recorder），或记录延期理由并给出后续 Issue 编号。
  - 依 REVIEW 的 Independent validation，本轮未重写已确认正确的部分：范围划分、
    T-068、C3／C4／C5 与 68 项活动任务计数保持不变。
