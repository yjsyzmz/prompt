...
- [x] **T-066 [Test/Fix] 把 deadline 收紧为「不得在 `now >= deadline` 时开始新回读」（MUST，Finding 2）。** **执行结果（2026-08-19）：已修复并转绿，253 项测试、0 skipped、0 failures。** 现状：1200ms deadline 只约束休眠累计，不约束「何时允许开始下一次回读」——代码在回读失败后才检查时钟，可能刚好休眠到 deadline，下一轮仍会先执行 A1／A3／A4 与 AX readback；真实 AX 查询耗时完全落在预算之外。既有 `testTheDeadlineStopsTheLoopIndependentlyOfTheAttemptCeiling` 还把「到达 300ms deadline 后执行第 4 次回读」**写成了期望结果**，该断言固定的是错误行为，须一并更正。**依 Solar 对 `b1cf0f2` 的 REVIEW MUST 2，边界拆成两个层次，测试须分别对应，不得一边断言总 elapsed 绝不超限、一边允许它超限：**
