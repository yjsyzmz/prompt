...
12. **分级诊断经 `os_log` 输出**：`replacement-stage` 记录会进入统一日志。
    **T-052 已完全删除长度字段**，`ReplacementStageReport` 现在既无 `String`
    也无数值成员，只有阶段标识、`DomainFailure` 分类，以及 A2 独有的
    「聚焦应用是否为本进程」布尔值。原先以「风险披露」保留长度元数据的做法
    已被 Solar 判定违反 FR-013／NFR-006 并撤回。代价：B1 阶段不再输出量级信息。
    详见 T-052。
...
长度元数据已由 T-052 删除），以及**第 13、14 项——脚本驱动的闭环证据不覆盖
