...
- `AccessibilityGateway` 直接实现 monitor ingress，在既有 actor 内只接受当前 session/target、忽略过期 callback；有效事件只能调用 `disableDirectActions`。独立 `AXMonitorEventRouter` 保留为同契约的轻量可测路由。
...
随后补充 workspace 投递、直接进入 `AccessibilityGateway` actor 与幂等释放三项测试，连续两次执行：

```text
./scripts/unit-tests.sh
Run 1: 77 tests, 0 failures — TEST SUCCEEDED
Run 2: 77 tests, 0 failures — TEST SUCCEEDED
```

其中 `AXObserverDeliveryIsolationTests` 共 9 项，全部通过：
...
5. callback 可直接异步进入 `AccessibilityGateway` actor；
6. 过期 callback 被 actor 忽略；
7. 当前 callback 只能禁用直接操作；
8. 重复停止只释放一次 source 与 workspace observation；
9. 生产监控源码不含任何写入或恢复授权符号。
